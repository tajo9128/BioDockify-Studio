...
def md_analysis_rmsd(req: MDAnalysisRequest):
    ...
    job = MD_JOBS.get(req.job_id)
    if not job or job.get("status") != "completed":
        return {
            "success": False,
            "error": f"Job {req.job_id} not found or not completed",
        }
    ...
    if not trajectory_path:
        return {
            "success": False,
            "error": "No trajectory file available for this job",
        }

    try:
        from md_analysis import RMSDAnalyzer
        analyzer = RMSDAnalyzer()
        rmsd_values = analyzer.analyze_rmsd(trajectory_path)
        plot_data = {
            "data": [{
        ...
        return {
            "success": False,
            "error": "MD analysis module not available",
        }
    except Exception as e:
