...
### Fixed
- **Installer Build**: Added missing section comment before `SkipDocker` function declaration.
...
### Fixed
- **Installer Build**: Corrected `MessageBox` syntax in `installer.nsi` (missing flags `MB_OK|MB_ICONSTOP`).
...
### Fixed
- **Installer Build**: Removed invalid `${NSD_DestroyDialog}` command from `installer.nsi` (dialog destruction is automatic).
